...
//
        ...
        for(int i=1;i<=4;i++){
             ...
             postRepository.save(post);
//같은 게시글에 같은 사용자가 3개의 댓글을 달았따
             Comment comment = Comment.builder()
                     .commentContent(i+"번째 게시글의 "+i+" 댓글입니다")
                     .post(post)
                     .user(users)
                     .build();
             Comment comment1 = Comment.builder()
...
//자신이 작성한 게시글에 본인이 좋아요를 눌렀따
            ...
            followService.makeFriend(2L,1L);
            followService.makeFriend(3L,1L);
    ...
    void findAllPostByUserId(){ //좋아요 개수에 distinct를 해주는 이유 : 조인시 댓글의 수만큼 중복 발생
        List<UserPostOneDto> allPostByUserId = postRepository.findAllPostByUserId(1L);
